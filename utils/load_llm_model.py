...
def prepare_to_load_model(username, service='huggingface', use_personal=False):
    ...
    if service.lower() == 'huggingface':
        ...
        apikey_filename = 'openai_api_key.txt'
        if use_personal:
            userinput = input("ARE YOU SURE YOU WANT TO USE YOUR PERSONAL OPENAI API KEY? y/N")
            if userinput != 'y':
                raise ValueError
            else:
                apikey_filename = 'personal_' + apikey_filename
        api_key_loc = os.path.join('/home', username, '.apikeys', apikey_filename)
